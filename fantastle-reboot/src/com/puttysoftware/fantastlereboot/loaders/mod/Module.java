...
public class Module {
  ...
  private static final int[] keyToPeriod = { 29020, 27392, 25855, 24403, 23034,
      21741, 20521, 19369, 18282, 17256, 16287, 15373, 14510, 13696 };
      ...
      if (sixteenBit) {
        if (signedSamples) {
          for (int idx = 0, end = sampleData.length; idx < end; idx++) {
            sampleData[idx] = (short) (moduleData[sampleOffset] & 0xFF
                | moduleData[sampleOffset + 1] << 8);
            sampleOffset += 2;
          }
        } else {
          for (int idx = 0, end = sampleData.length; idx < end; idx++) {
  ...
  private void loadXM(final byte[] moduleData) {
    ...
    for (int patIdx = 0; patIdx < this.numPatterns; patIdx++) {
      ...
      final int patternDataLength = Module.ushortle(moduleData, dataOffset + 7);
      ...
      if (patternDataLength > 0) {
        ...
        for (int note = 0; note < numNotes; note++) {
          ...
          final byte key = (flags & 0x01) > 0 ? moduleData[dataOffset++] : 0;
          pattern.data[patternDataOffset++] = key;
          final byte ins = (flags & 0x02) > 0 ? moduleData[dataOffset++] : 0;
          pattern.data[patternDataOffset++] = ins;
          final byte vol = (flags & 0x04) > 0 ? moduleData[dataOffset++] : 0;
    ...
    for (int insIdx = 1; insIdx <= this.numInstruments; insIdx++) {
      ...
      final int numSamples = instrument.numSamples = Module.ushortle(moduleData,
          dataOffset + 27);
      if (numSamples > 0) {
      ...
      for (int samIdx = 0; samIdx < numSamples; samIdx++) {
        ...
        sample.name = Module.codePage850(moduleData, sampleHeaderOffset + 18,
            22);
        ...
        sample.setSampleData(sampleData, sampleLoopStart, sampleLoopLength,
            pingPong);
