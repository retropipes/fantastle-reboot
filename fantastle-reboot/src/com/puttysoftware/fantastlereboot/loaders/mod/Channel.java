...
public class Channel {
  ...
  private static final int[] periodTable = {
      ...
      54784, 54390, 53999, 53610, 53224, 52841, 52461, 52084, 51709, 51337,
      50968, 50601, 50237, 49876, 49517, 49161, 48807, 48456, 48107, 47761,
      47418, 47076, 46738, 46401, 46068, 45736, 45407, 45081, 44756, 44434,
      44115, 43797, 43482, 43169, 42859, 42550, 42244, 41940, 41639, 41339,
      41042, 40746, 40453, 40162, 39873, 39586, 39302, 39019, 38738, 38459,
      38183, 37908, 37635, 37365, 37096, 36829, 36564, 36301, 36040, 35780,
      35523, 35267, 35014, 34762, 34512, 34263, 34017, 33772, 33529, 33288,
      33049, 32811, 32575, 32340, 32108, 31877, 31647, 31420, 31194, 30969,
      30746, 30525, 30306, 30088, 29871, 29656, 29443, 29231, 29021, 28812,
      28605, 28399, 28195, 27992, 27790, 27590, 27392, 27195, 26999, 26805,
      26612, 26421, 26231, 26042 };
  private static final int[] freqTable = {
      // Frequency for keys 109 to 121 with 8 fractional values.
      267616, 269555, 271509, 273476, 275458, 277454, 279464, 281489, 283529,
      285584, 287653, 289738, 291837, 293952, 296082, 298228, 300389, 302566,
      304758, 306966, 309191, 311431, 313688, 315961, 318251, 320557, 322880,
      325220, 327576, 329950, 332341, 334749, 337175, 339618, 342079, 344558,
      347055, 349570, 352103, 354655, 357225, 359813, 362420, 365047, 367692,
      370356, 373040, 375743, 378466, 381209, 383971, 386754, 389556, 392379,
      395222, 398086, 400971, 403877, 406803, 409751, 412720, 415711, 418723,
      421758, 424814, 427892, 430993, 434116, 437262, 440430, 443622, 446837,
      450075, 453336, 456621, 459930, 463263, 466620, 470001, 473407, 476838,
      480293, 483773, 487279, 490810, 494367, 497949, 501557, 505192, 508853,
      512540, 516254, 519995, 523763, 527558, 531381, 535232, 539111, 543017,
      546952, 550915, 554908, 558929, 562979 };
  private static final short[] arpTuning = { 4096, 4340, 4598, 4871, 5161, 5468,
      5793, 6137, 6502, 6889, 7298, 7732, 8192, 8679, 9195, 9742 };
  private static final short[] sineTable = { 0, 24, 49, 74, 97, 120, 141, 161,
      180, 197, 212, 224, 235, 244, 250, 253, 255, 253, 250, 244, 235, 224, 212,
  ...
  public Channel(final Module newModule, final int id, final int newSampleRate,
      final GlobalVol newGlobalVol) {
  ...
  public void resample(final int[] outBuf, final int offset, final int length,
    ...
    case NEAREST:
      this.sample.resampleNearest(this.sampleIdx, this.sampleFra, this.step,
          lAmpl, rAmpl, outBuf, offset, length);
      break;
    case LINEAR:
    default:
      this.sample.resampleLinear(this.sampleIdx, this.sampleFra, this.step,
          lAmpl, rAmpl, outBuf, offset, length);
      break;
    case SINC:
      this.sample.resampleSinc(this.sampleIdx, this.sampleFra, this.step, lAmpl,
          rAmpl, outBuf, offset, length);
      break;
    }
  }
  ...
  private void updateEnvelopes() {
    if (this.instrument.volumeEnvelope.enabled) {
      ...
      this.volEnvTick = this.instrument.volumeEnvelope.nextTick(this.volEnvTick,
          this.keyOn);
    }
  ...
  private void vibrato(final boolean fine) {
    this.vibratoAdd = this.waveform(this.vibratoPhase, this.vibratoType & 0x3)
        * this.vibratoDepth >> (fine ? 7 : 5);
  }

  private void tremolo() {
    this.tremoloAdd = this.waveform(this.tremoloPhase, this.tremoloType & 0x3)
        * this.tremoloDepth >> 6;
  }
  ...
  private void calculateFrequency() {
    if (this.module.linearPeriods) {
      ...
      if (freq < 65536) {
        ...
        this.step = (freq << Sample.FP_SHIFT - 3) / (this.sampleRate >> 3);
      }
    } else {
