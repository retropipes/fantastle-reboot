...
public class EffectImageLoader {
	...
	if (image != EffectImageIndex._NONE) {
	    ...
	    final String name = "/assets/images/effects/" + EffectImageLoader.allFilenames[image.ordinal()] + imageExt;
	    return DianeImageLoader.load(name, EffectImageLoader.class.getResource(name));
	}
	return null;
    }
	...
	try (InputStream stream = EffectImageLoader.class
	...
	for (int i = 0; i <= EffectImageLoader.MAX_INDEX; i++) {
	    final String name = "/assets/images/effects/" + EffectImageLoader.allFilenames[i] + imageExt;
	    try {
		DianeImageLoader.load(name, EffectImageLoader.class.getResource(name));
