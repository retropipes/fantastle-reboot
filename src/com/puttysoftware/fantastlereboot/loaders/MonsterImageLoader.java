...
public class MonsterImageLoader {
    ...
    static BufferedImageIcon loadUncached(final String name, final ColorShader shader) {
	try {
	    final URL url = MonsterImageLoader.class.getResource("/assets/images/monsters/" + name);
	...
	final String imageExt = MonsterImageLoader.fileExtensions.getProperty("images");
	final String cacheName = shader.getName() + "_" + Integer.toString(imageID);
	return ImageCache.getCachedImage(shader, Integer.toString(imageID) + imageExt, cacheName);
    }
	...
	// Methods
	public static BufferedImageIcon getCachedImage(final ColorShader shader, final String imagePath,
		final String name) {
	    ...
	    final BufferedImageIcon newImage = MonsterImageLoader.loadUncached(imagePath, shader);
