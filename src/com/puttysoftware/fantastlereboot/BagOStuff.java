...
import org.retropipes.diane.update.BrowserLauncher;
...
public class BagOStuff {
    ...
    void postConstruct() {
	try {
	    BagOStuff.pd = new ProductData(BagOStuff.UPDATE_SITE, BagOStuff.NEW_VERSION_SITE, BagOStuff.VERSION_MAJOR,
		    BagOStuff.VERSION_MINOR, BagOStuff.VERSION_BUGFIX, BagOStuff.VERSION_CODE,
	...
	try {
	    final UpdateCheckResults results = Prefs.checkForUpdates(manual, BagOStuff.pd);
	    if (results.hasUpdate()) {
		final int resp = CommonDialogs.showConfirmDialog("An update is available. Do you want to download it?",
	    ...
	    if (manual) {
		CommonDialogs.showDialog("The check for updates could not be completed,\n"
