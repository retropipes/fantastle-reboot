...
final class MovementTask extends Thread {
	...
	try {
	    while (true) {
		this.waitForWork();
		MovementTask.updatePositionRelative(MovementTask.moveX, MovementTask.moveY, MovementTask.moveZ);
	    }
    ...
    private static void fireStepActions(final int px, final int py, final int pz) {
	...
	final FantastleObjectModel obj = m.getCell(px, py, pz, Layers.OBJECT);
    ...
    private static void updatePositionRelative(final int dirX, final int dirY, final int dirZ) {
	...
	do {
		...
		if (m.cellRangeCheck(px + fX, py + fY, pz + fZ)) {
		    if (MovementTask.checkSolid(MovementTask.saved, below, nextBelow, nextAbove)) {
    ...
    private static boolean checkLoopCondition(final FantastleObjectModel below, final FantastleObjectModel nextBelow,
	    final FantastleObjectModel nextAbove) {
	return MovementTask.proceed && !EffectManager.isEffectActive(EffectConstants.EFFECT_STICKY)
		&& !nextBelow.hasFriction() && MovementTask.checkSolid(MovementTask.saved, below, nextBelow, nextAbove);
    }

    private static boolean checkSolid(final FantastleObjectModel inside, final FantastleObjectModel below,
    ...
    private static void updatePositionAbsolute(final int x, final int y, final int z) {
	final World m = WorldManager.getWorld();
	m.savePlayerLocation();
	GameView.saveViewingWindow();
	if (m.cellRangeCheck(x, y, z) && !m.getCell(x, y, z, Layers.OBJECT).isSolid()) {
	    m.setPlayerLocationX(x);
	    m.setPlayerLocationY(y);
	    m.setPlayerLocationZ(z);
	    GameView.setViewingWindowLocationX(m.getPlayerLocationY() - GameView.getOffsetFactorX());
	    GameView.setViewingWindowLocationY(m.getPlayerLocationX() - GameView.getOffsetFactorY());
	    MovementTask.saved = m.getCell(m.getPlayerLocationX(), m.getPlayerLocationY(), m.getPlayerLocationZ(),
		    Layers.OBJECT);
	    FileStateManager.setDirty(true);
    ...
    private static void checkFloorChange(final int px, final int py, final int pz) {
