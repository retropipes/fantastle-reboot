...
class GameCanvas extends JPanel {
	...
	for (x = viewX; x <= viewLRX; x++) {
		...
		inBounds = x >= 0 && x < World.getMaxRows() && y >= 0 && y < World.getMaxColumns();
		if (inBounds) {
		    if (visible) {
			final FantastleObjectModel obj1 = m.getCell(y, x, m.getPlayerLocationZ(), Layers.GROUND);
			final FantastleObjectModel obj2 = m.getCell(y, x, m.getPlayerLocationZ(), Layers.OBJECT);
			...
			final String cacheName = GameCanvas.generateCacheName(obj1, obj2, obj3, obj4);
		    ...
		    this.drawGrid.setImageCell(GameCanvas.NOTHING.getGameImage(), xFix, yFix);
		}
	    }
	}
