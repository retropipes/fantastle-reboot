...
final class EditorView {
	...
	if (EditorView.locX >= EditorView.getMinX() && EditorView.locX <= EditorView.getMaxX()
		&& EditorView.locY >= EditorView.getMinY() && EditorView.locY <= EditorView.getMaxY()) {
	    return true;
	} else {
	    return false;
	}
	...
	final int minX = EditorView.getMinX();
	if (EditorView.locX < minX) {
	    EditorView.locX = minX;
	}
	final int maxX = EditorView.getMaxX();
	if (EditorView.locX > maxX) {
	    EditorView.locX = maxX;
	}
	final int minY = EditorView.getMinY();
	if (EditorView.locY < minY) {
	    EditorView.locY = minY;
	}
	final int maxY = EditorView.getMaxY();
