...
public class Editor {
	...
	Editor.groundEditorAppearances = GameObjects.getAllGroundLayerEditorAppearances();
	Editor.objectEditorAppearances = GameObjects.getAllObjectLayerEditorAppearances();
    ...
    public static void updateEditorPosition(final int x, final int y, final int z, final int w) {
	...
	Editor.outputFrame.pack();
	Editor.outputFrame.setTitle("Editor (Ground Layer) - Floor " + (z + 1) + " Level " + (w + 1));
    }
	...
	Editor.secondaryPane.repaint();
	Editor.outputFrame.pack();
	Editor.outputFrame.setTitle("Editor (Object Layer) - Floor " + (z + 1) + " Level " + (w + 1));
    }
	...
	final int xOffset = Editor.vertScroll.getValue() - Editor.vertScroll.getMinimum();
	final int yOffset = Editor.horzScroll.getValue() - Editor.horzScroll.getMinimum();
	final int gridX = x / ImageConstants.SIZE + EditorView.getLocX() - xOffset + yOffset;
	final int gridY = y / ImageConstants.SIZE + EditorView.getLocY() + xOffset - yOffset;
	...
	if (world.cellRangeCheck(gridX, gridY, gridZ)) {
	    Editor.updateUndoHistory(Editor.savedObject, gridX, gridY, gridZ, EditorLoc.getLocW(), gridE);
	    world.setCell(mo, gridX, gridY, gridZ, gridE);
	    Editor.checkStairPair(gridZ);
    ...
    public static void editObjectProperties() {
	Game.setStatusMessage("This object has no properties");
    }

    private static void checkStairPair(final int z) {
	final FantastleObjectModel obj1 = WorldManager.getWorld().getCell(EditorLoc.getLocX(), EditorLoc.getLocY(), z,
		Layers.OBJECT);
	final FantastleObjectModel obj2 = WorldManager.getWorld().getCell(EditorLoc.getLocX(), EditorLoc.getLocY(),
		z + 1, Layers.OBJECT);
	final FantastleObjectModel obj3 = WorldManager.getWorld().getCell(EditorLoc.getLocX(), EditorLoc.getLocY(),
		z - 1, Layers.OBJECT);
	if (!(obj1 instanceof StairsUp)) {
	    if (obj2 instanceof StairsDown) {
		Editor.unpairStairs(Editor.STAIRS_UP, z);
	    } else if (!(obj1 instanceof StairsDown)) {
		if (obj3 instanceof StairsUp) {
		    Editor.unpairStairs(Editor.STAIRS_DOWN, z);
		}
	    }
	}
    }

    private static void reverseCheckStairPair(final int z) {
	final FantastleObjectModel obj1 = WorldManager.getWorld().getCell(EditorLoc.getLocX(), EditorLoc.getLocY(), z,
		Layers.OBJECT);
	final FantastleObjectModel obj2 = WorldManager.getWorld().getCell(EditorLoc.getLocX(), EditorLoc.getLocY(),
		z + 1, Layers.OBJECT);
	...
	if (obj1 instanceof StairsUp) {
	    if (!(obj2 instanceof StairsDown)) {
		Editor.pairStairs(Editor.STAIRS_UP, z);
	    } else if (obj1 instanceof StairsDown) {
		if (!(obj3 instanceof StairsUp)) {
		    Editor.pairStairs(Editor.STAIRS_DOWN, z);
		}
	    }
	}
    }

    private static void pairStairs(final int type, final int z) {
	final World world = WorldManager.getWorld();
    ...
    private static void unpairStairs(final int type, final int z) {
	final World world = WorldManager.getWorld();
	final int locX = EditorLoc.getLocX();
	final int locY = EditorLoc.getLocY();
	switch (type) {
	...
	input1 = CommonDialogs.showTextInputDialogWithDefault(
		"Number of rows? (minimum 2, maximum " + absoluteRCLimit + ")", msg, Integer.toString(absoluteRCLimit));
	if (input1 != null) {
	    input2 = CommonDialogs.showTextInputDialogWithDefault(
		    "Number of columns? (minimum 2, maximum " + absoluteRCLimit + ")", msg,
		    Integer.toString(absoluteRCLimit));
	    if (input2 != null) {
		input3 = CommonDialogs.showTextInputDialogWithDefault(
			"Number of floors? (minimum 1, maximum " + absoluteFLimit + ")", msg,
			Integer.toString(absoluteFLimit));
		if (input3 != null) {
		    try {
			...
			success = WorldManager.getWorld().addLevel(levelSizeX, levelSizeY, levelSizeZ);
			if (success) {
			    Editor.fixLimits();
			    if (!flag) {
	...
	Editor.outputFrame = MainWindow.mainWindow();
	...
	Editor.secondaryPane.setLayout(new GridLayout(EditorView.getSizeX(), EditorView.getSizeY()));
	Editor.horzScroll = new JScrollBar(Adjustable.HORIZONTAL, EditorView.getMinY(), EditorView.getSizeY(),
		EditorView.getMinY(), EditorView.getMaxY());
	Editor.vertScroll = new JScrollBar(Adjustable.VERTICAL, EditorView.getMinX(), EditorView.getSizeX(),
		EditorView.getMinX(), EditorView.getMaxX());
	Editor.c.gridx = 0;
	Editor.c.gridy = 0;
	...
	Editor.secondaryPane.setPreferredSize(new Dimension(vSize * gSize, vSize * gSize));
    }
	...
	if (Editor.engine.isDataValid() && world.cellRangeCheck(x, y, z, w, e)) {
		...
		Editor.checkStairPair(z);
	    } else {
		Editor.reverseCheckStairPair(z);
	    }
    ...
    private static void updateUndoHistory(final FantastleObjectModel obj, final int x, final int y, final int z,
	    final int w, final int e) {
	Editor.engine.updateUndoHistory(obj, x, y, z, w, e);
    }

    private static void updateRedoHistory(final FantastleObjectModel obj, final int x, final int y, final int z,
	    final int w, final int e) {
    ...
    private static class EventHandler implements AdjustmentListener, MouseListener, WindowListener {
	public EventHandler() {
	    super();
	}
