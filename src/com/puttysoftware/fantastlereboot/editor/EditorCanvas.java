...
class EditorCanvas extends JPanel {
	...
	final boolean playerInBounds = m.cellRangeCheck(u, v, m.getPlayerLocationZ());
	for (x = viewX; x <= viewLRX; x++) {
		...
		if (inBounds) {
			...
			final String cacheName = EditorCanvas.generateCacheName(obj1, obj2, obj3, obj4);
			this.drawGrid.setImageCell(ImageCompositor.composite(cacheName, img1, img2, img3, img4), xFix,
				yFix);
		    } else {
			final String cacheName = EditorCanvas.generateCacheName(obj1, obj3, obj4);
			this.drawGrid.setImageCell(ImageCompositor.composite(cacheName, img1, img3, img4), xFix, yFix);
		    }
		} else {
		    this.drawGrid.setImageCell(EditorCanvas.NOTHING.getEditorImage(), xFix, yFix);
		}
	    }
	}
