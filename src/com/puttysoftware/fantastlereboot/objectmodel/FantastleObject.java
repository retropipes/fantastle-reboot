...
public abstract class FantastleObject extends GameObject implements FantastleObjectModel {
    ...
    public FantastleObject(final int objectID, final String cacheName, final ObjectImageIndex image) {
    ...
    @Override
    public void setGameLook(final String cacheName, final ObjectImageIndex image, final ColorShader shader) {
	super.setGameLook(new ObjectAppearance(cacheName, image, shader));
    }

    @Override
    public void setEditorLook(final String cacheName, final ObjectImageIndex image) {
	super.setEditorLook(new ObjectAppearance(cacheName, image));
    }

    @Override
    public void setEditorLook(final String cacheName, final ObjectImageIndex image, final ColorShader shader) {
	super.setEditorLook(new ObjectAppearance(cacheName, image, shader));
    }

    @Override
    public void setBattleLook(final String cacheName, final ObjectImageIndex image) {
	super.setBattleLook(new ObjectAppearance(cacheName, image));
    }

    @Override
    public void setBattleLook(final String cacheName, final ObjectImageIndex image, final ColorShader shader) {
    ...
    @Override
    public boolean shouldGenerateObject(final World world, final int row, final int col, final int floor,
    ...
    @Override
    public final void writeObject(final DataIOWriter writer) throws IOException {
    ...
    @Override
    public final FantastleObjectModel readObject(final DataIOReader reader, final int uid) throws IOException {
	if (uid == this.getUniqueID()) {
	    final int savedIdent = reader.readInt();
	    if (savedIdent != -1) {
		this.savedObject = GameObjects.readSavedObject(reader, savedIdent, WorldVersions.LATEST);
	    }
	    final int cc = this.customCountersLength();
	    for (int x = 0; x < cc; x++) {
