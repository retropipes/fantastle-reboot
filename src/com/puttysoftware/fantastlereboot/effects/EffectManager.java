...
import com.puttysoftware.fantastlereboot.directions.DirectionsResolver;
...
public class EffectManager {
	...
	EffectManager.activeEffects[EffectConstants.EFFECT_ROTATED_CLOCKWISE] = new RotatedCW(0);
	EffectManager.activeEffects[EffectConstants.EFFECT_ROTATED_COUNTERCLOCKWISE] = new RotatedCCW(0);
	EffectManager.activeEffects[EffectConstants.EFFECT_U_TURNED] = new UTurned(0);
	EffectManager.activeEffects[EffectConstants.EFFECT_CONFUSED] = new Confused(0);
	...
	EffectManager.activeEffects[EffectConstants.EFFECT_POWER_GATHER] = new PowerGather(0);
	EffectManager.activeEffects[EffectConstants.EFFECT_POWER_WITHER] = new PowerWither(0);
	// Create GUI
	...
	EffectManager.activeEffectMessageJPanel.setLayout(new GridLayout(EffectManager.MAX_ACTIVE_EFFECTS, 1));
	for (int z = 0; z < EffectManager.MAX_ACTIVE_EFFECTS; z++) {
	    EffectManager.activeEffectMessages[z] = new JLabel("");
	    EffectManager.activeEffectMessageJPanel.add(EffectManager.activeEffectMessages[z]);
	}
	...
	if (index != -1) {
		...
		EffectManager.activeEffectMessages[z].setText(EffectManager.activeEffectMessages[z + 1].getText());
		EffectManager.activeEffectIndices[z] = EffectManager.activeEffectIndices[z + 1];
	    }
	    // Clear last entry
	...
	final int index = EffectManager.lookupEffect(effectID);
	if (index != -1) {
	    final String effectString = EffectManager.activeEffects[effectID].getEffectString();
