...
final class LayeredTower implements Cloneable {
    ...
    public LayeredTower(final int rows, final int cols, final int floors, final long randSeed) {
	this.data = new LowLevelWorldDataStore(cols, rows, floors, Layers.COUNT);
	this.monsterData = new FlagStorage(cols, rows, floors);
	this.savedTowerState = new LowLevelWorldDataStore(cols, rows, floors, Layers.COUNT);
    ...
    private boolean checkMoveMonster(final int locX, final int locY, final int locZ, final int moveX, final int moveY) {
	if (this.monsterData.getCell(locY, locX, locZ) && !this.monsterData.getCell(locY + moveY, locX + moveX, locZ)) {
	    return true;
	}
	return false;
    }

    private void moveMonster(final int locX, final int locY, final int locZ, final int moveX, final int moveY) {
	...
	for (locX = 0; locX < cols; locX++) {
		...
		if (this.hasMonster(locX, locY, pz)) {
		    if (locX + moveX >= 0 && locX + moveX < cols && locY + moveY >= 0 && locY + moveY < rows) {
			final FantastleObjectModel there = world.getCell(locX + moveX, locY + moveY, pz, Layers.OBJECT);
			final boolean checkMove = this.checkMoveMonster(locX, locY, pz, moveX, moveY);
	...
	return fR >= 0 && fR < this.getRows() && fC >= 0 && fC < this.getColumns() && fF >= 0 && fF < this.getFloors();
    }

    public boolean cellRangeCheck(final int row, final int col, final int floor, final int extra) {
	int fR = row;
	int fC = col;
	final int fF = floor;
	if (this.verticalWrapEnabled) {
	...
	return fR >= 0 && fR < this.getRows() && fC >= 0 && fC < this.getColumns() && fF >= 0 && fF < this.getFloors()
		&& extra >= 0 && extra < Layers.COUNT;
    }

    public FantastleObjectModel getCell(final int row, final int col, final int floor, final int extra) {
	int fR = row;
	int fC = col;
	int fF = floor;
	if (this.verticalWrapEnabled) {
	...
	if ((this.visionMode | VisionModes.EXPLORE) == this.visionMode) {
		...
		for (int y = yp - this.visionModeExploreRadius; y <= yp + this.visionModeExploreRadius; y++) {
		    ...
		    if (!alreadyVisible) {
			if ((this.visionMode | VisionModes.FIELD_OF_VIEW) == this.visionMode) {
	...
	if (this.visionMode == VisionModes.NONE) {
		...
		if (result && (this.visionMode | VisionModes.FIELD_OF_VIEW) == this.visionMode) {
	...
	return fx1 >= -1 && fx1 <= this.getRows() && fx2 >= -1 && fx2 <= this.getRows() && fy1 >= -1
		&& fy1 <= this.getColumns() && fy2 >= -1 && fy2 <= this.getColumns();
    }
    ...
    private boolean isSquareVisibleFieldOfView(final int x1, final int y1, final int x2, final int y2) {
	int fx1, fx2, fy1, fy2;
	fx1 = x1;
	fx2 = x2;
	fy1 = y1;
	...
	do {
		...
		final FantastleObjectModel obj = this.getCell(fx1, fy1, zLoc, Layers.OBJECT);
    ...
    private boolean isSquareVisibleFixedRadius(final int x1, final int y1, final int x2, final int y2) {
	int fx1, fy1, fx2, fy2;
	if (this.isHorizontalWrapEnabled()) {
	    fx1 = this.normalizeColumn(x1);
	    fx2 = this.normalizeColumn(x2);
	...
	final int zLoc = this.getPlayerFloor();
	if (this.cellRangeCheck(fx1, fy1, zLoc) && this.cellRangeCheck(fx2, fy2, zLoc)) {
    ...
    public void setCell(final FantastleObjectModel mo, final int row, final int col, final int floor, final int extra) {
	int fR = row;
	int fC = col;
	int fF = floor;
	if (this.verticalWrapEnabled) {
    ...
    private void fillFloor(final FantastleObjectModel bottom, final FantastleObjectModel top, final int z) {
	int x, y, e;
    ...
    private void fillFloorRandomlyConstrained(final World world, final int z, final int w) {
	...
	for (e = 0; e < Layers.COUNT; e++) {
		...
		for (x = 0; x < columns; x++) {
		    for (y = 0; y < rows; y++) {
			final FantastleObjectModel placeObj = objectsWithoutPrerequisites[r.generate()];
			final boolean okay = placeObj.shouldGenerateObject(world, x, y, z, w, e);
			if (okay) {
			    this.setCell(GameObjects.getNewInstanceByUniqueID(placeObj.getUniqueID()), y, x, z, e);
			}
		    }
		}
	    }
	...
	for (int layer = 0; layer < Layers.COUNT; layer++) {
	    final FantastleObjectModel[] requiredObjects = GameObjects.getAllRequired(layer);
		...
		for (x = 0; x < requiredObjects.length; x++) {
			...
			if (currObj.shouldGenerateObject(world, randomRow, randomColumn, z, w, layer)) {
			    this.setCell(GameObjects.getNewInstanceByUniqueID(currObj.getUniqueID()), randomColumn,
				    randomRow, z, layer);
			} else {
			    while (!currObj.shouldGenerateObject(world, randomColumn, randomRow, z, w, layer)) {
				randomRow = row.generate();
				randomColumn = column.generate();
			    }
			    this.setCell(GameObjects.getNewInstanceByUniqueID(currObj.getUniqueID()), randomColumn,
    ...
    void fillRandomlyTwister(final int w) {
	for (int z = 0; z < this.getFloors(); z++) {
	    this.fillFloorRandomlyTwister(z, w);
	}
    }

    private void fillFloorRandomlyTwister(final int z, final int w) {
	...
	this.rectFill(0, 0, this.size.width, this.size.height, "#");
	// Create a starting room
	this.startingRoom(w);
	// Generate halls/rooms until no prospects remain
	while (this.hasProspects()) {
	    this.hallsGenerate();
	    this.roomsGenerate();
	}
    ...
    private void rectFill(final int x, final int y, final int w, final int h, final String tile) {
	...
	final int center = this.size.width / 2;
	this.rectFill(center - 2, 1, 3, 3, "R");
	this.placeIfWall(center - 1, 4, "P");
	if (w != 0) {
	    this.placeIfEmpty(center, 2, "s");
	}
    }
	...
	if (w != World.getMaxLevels() - 1) {
	    try {
		this.placeIfEmpty(this.lastRoomX + this.lastRoomW - 2, this.lastRoomY + this.lastRoomH - 2, "S");
	    } catch (final ArrayIndexOutOfBoundsException e) {
		this.placeIfEmpty(this.lastRoomX + 1, this.lastRoomY + 1, "S");
	    }
	}
    }

    private boolean rectCheck(final int x, final int y, final int w, final int h) {
	for (int ya = y; ya < y + h; ya++) {
	    for (int xa = x; xa < x + w; xa++) {
		String tile;
		try {
    ...
    private void placeIfWall(final int x, final int y, final String tile) {
	if ("#".equals(this.tiles[x][y])) {
	    this.tiles[x][y] = tile;
	}
    }

    private void placeIfEmpty(final int x, final int y, final String tile) {
	if (LayeredTower.isEmpty(this.tiles[x][y])) {
	    this.tiles[x][y] = tile;
	}
    }
	...
	final ArrayList<Point> prospects = new ArrayList<>();
	for (int y = 0; y < this.size.height; y++) {
	    for (int x = 0; x < this.size.width; x++) {
		if ("P".equals(this.tiles[x][y])) {
	...
	this.getProspects().stream().forEach((p) -> {
	    try {
		this.hallMake(p.x, p.y);
	    } catch (final ArrayIndexOutOfBoundsException e) {
		this.tiles[p.x][p.y] = "#";
	    }
	});
    ...
    private void hallMake(final int x, final int y) {
	if (LayeredTower.isEmpty(this.tiles[x - 1][y]) && LayeredTower.isEmpty(this.tiles[x + 1][y])) {
	    this.tiles[x][y] = "H";
	    return;
	}
	if (LayeredTower.isEmpty(this.tiles[x][y - 1]) && LayeredTower.isEmpty(this.tiles[x][y + 1])) {
	    this.tiles[x][y] = "H";
	    return;
	}
	if (LayeredTower.isEmpty(this.tiles[x - 1][y])) {
	    this.hallMakeEastbound(x, y, this.rand.nextInt(this.hallsize));
	    return;
	}
	if (LayeredTower.isEmpty(this.tiles[x + 1][y])) {
	    this.hallMakeWestbound(x, y, this.rand.nextInt(this.hallsize));
	    return;
	}
	if (LayeredTower.isEmpty(this.tiles[x][y - 1])) {
	    this.hallMakeSouthbound(x, y, this.rand.nextInt(this.hallsize));
	    return;
	}
	if (LayeredTower.isEmpty(this.tiles[x][y + 1])) {
	    this.hallMakeNorthbound(x, y, this.rand.nextInt(this.hallsize));
	    return;
	}
	this.tiles[x][y] = "R";
    }

    private void hallMakeEastbound(final int x, final int y, final int length) {
	if (this.rectCheck(x, y - 1, length, 3)) {
	    this.rectFill(x, y, length, 1, "H");
	    this.placeIfEmpty(x + length - 1, y, "P");
	} else {
	    this.tiles[x][y] = "#";
	}
    }

    private void hallMakeWestbound(final int x, final int y, final int length) {
	if (this.rectCheck(x - length, y - 1, length, 3)) {
	    this.rectFill(x - (length - 1), y, length, 1, "H");
	    this.placeIfEmpty(x - (length - 1), y, "P");
	    if (length >= 3) {
		final int sbranch = this.rand.nextInt(this.hallsize);
		if (sbranch > 1 && sbranch < length - 1) {
		    this.placeIfEmpty(x - sbranch, y + 1, "P");
		    this.hallMakeSouthbound(x - sbranch, y + 1, this.rand.nextInt(this.hallsize));
		}
		final int nbranch = this.rand.nextInt(this.hallsize);
		if (nbranch > 1 && nbranch < length - 1) {
		    this.placeIfEmpty(x - nbranch, y - 1, "P");
		    this.hallMakeNorthbound(x - nbranch, y - 1, this.rand.nextInt(this.hallsize));
		}
	    }
	} else {
	    if (length > 0) {
		this.hallMakeWestbound(x, y, length - 1);
	    } else {
		this.tiles[x][y] = "#";
	    }
	}
    ...
    private void hallMakeSouthbound(final int x, final int y, final int length) {
	if (this.rectCheck(x - 1, y, 3, length + 1)) {
	    this.rectFill(x, y, 1, length, "H");
    ...
    private void hallMakeNorthbound(final int x, final int y, final int length) {
	if (this.rectCheck(x - 1, y - length, 3, length + 1)) {
	    this.rectFill(x, y - (length - 1), 1, length, "H");
	    this.tiles[x][y - (length - 1)] = "P";
	} else {
	    this.tiles[x][y] = "#";
	}
	...
	this.getProspects().stream().forEach((p) -> {
	    try {
		this.roomMake(p.x, p.y);
	    } catch (final ArrayIndexOutOfBoundsException e) {
	    }
	});
    }

    private void roomMake(final int x, final int y) {
	if (LayeredTower.isEmpty(this.tiles[x - 1][y]) && LayeredTower.isEmpty(this.tiles[x + 1][y])) {
	    this.tiles[x][y] = "R";
	    return;
	}
	...
	final int w = 3 + this.rand.nextInt(this.roomsize);
	final int h = 3 + this.rand.nextInt(this.roomsize);
	if (LayeredTower.isEmpty(this.tiles[x][y - 1])) {
	    this.roomMakeSouthbound(x, y, w, h);
	    this.lastRoomX = x;
	    this.lastRoomY = y;
	    this.lastRoomW = w;
	    this.lastRoomH = h;
	...
	if (LayeredTower.isEmpty(this.tiles[x - 1][y])) {
	    this.roomMakeEastbound(x, y, w, h);
	    this.lastRoomX = x;
	    this.lastRoomY = y;
	    this.lastRoomW = w;
	    this.lastRoomH = h;
	...
	if (LayeredTower.isEmpty(this.tiles[x + 1][y])) {
	    this.roomMakeWestbound(x, y, w, h);
	    this.lastRoomX = x;
	    this.lastRoomY = y;
	    this.lastRoomW = w;
	    this.lastRoomH = h;
	...
	if (LayeredTower.isEmpty(this.tiles[x][y + 1])) {
	    this.roomMakeNorthbound(x, y, w, h);
	    this.lastRoomX = x;
	    this.lastRoomY = y;
	    this.lastRoomW = w;
	    this.lastRoomH = h;
    ...
    private void roomMakeSouthbound(final int x, final int y, final int w, final int h) {
	final int wc = w / 2;
	final int hc = h / 2;
	final int xorig = x - wc;
	final int yorig = y + 1;
	if (this.rectCheck(xorig - 1, y, w + 1, h + 1)) {
	    this.tiles[x][y] = "H";
	    this.rectFill(xorig, yorig, w, h, "R");
	    this.placeIfWall(xorig + wc, yorig + h, "P");
	    this.placeIfWall(xorig - 1, yorig + hc, "P");
	    this.placeIfWall(xorig + w, yorig + hc, "P");
	} else {
	    this.tiles[x][y] = "#";
	}
    }

    private void roomMakeEastbound(final int x, final int y, final int w, final int h) {
	final int wc = w / 2;
	final int hc = h / 2;
	final int xorig = x + 1;
	final int yorig = y - hc;
	if (this.rectCheck(xorig, yorig - 2, w + 1, h + 1)) {
	    this.tiles[x][y] = "H";
	    this.rectFill(xorig, yorig, w, h, "R");
	    this.placeIfWall(xorig + wc, yorig - 1, "P");
	    this.placeIfWall(xorig + wc, y + hc, "P");
	    this.placeIfWall(xorig + w, y, "P");
	} else {
	    if (w > 3 && h > 3) {
		this.roomMakeEastbound(x, y, w - 1, h - 1);
	    } else {
		this.tiles[x][y] = "#";
	    }
	}
    ...
    private void roomMakeWestbound(final int x, final int y, final int w, final int h) {
	final int hc = h / 2;
	final int wc = w / 2;
	final int xorig = x - w;
	final int yorig = y - hc;
	if (this.rectCheck(xorig - 1, yorig - 1, w + 1, h + 1)) {
	    this.tiles[x][y] = "H";
	    this.rectFill(xorig, yorig, w, h, "R");
	    this.placeIfWall(xorig + wc, yorig - 1, "P");
	    this.placeIfWall(xorig + wc, y + hc, "P");
	    this.placeIfWall(xorig - 1, y, "P");
	} else {
	    this.tiles[x][y] = "#";
	}
    }

    private void roomMakeNorthbound(final int x, final int y, final int w, final int h) {
	final int wc = w / 2;
	final int hc = h / 2;
	final int xorig = x - wc;
	final int yorig = y - h;
	if (this.rectCheck(xorig - 1, yorig - 1, w + 1, h + 1)) {
	    this.tiles[x][y] = "H";
	    this.rectFill(xorig, yorig, w, h, "R");
	    this.placeIfWall(x, yorig - 1, "P");
	    this.placeIfWall(xorig - 1, y - hc, "P");
	    this.placeIfWall(xorig + w, y - hc, "P");
	} else {
	    this.tiles[x][y] = "#";
	}
    }
	...
	this.fixOneBlockDeadEnds();
	this.fixOneBlockHalls();
	this.makeDoors();
	// Convert tiles to dungeon data
	this.convertTilestoObjects(z);
    }

    private void fixOneBlockDeadEnds() {
	for (int x = 0; x < this.size.width; x++) {
	    for (int y = 0; y < this.size.height; y++) {
		final String tile = this.tiles[x][y];
		if ("H".equals(tile)) {
		    // Westbound
		    if ("R".equals(this.tiles[x + 1][y]) && "#".equals(this.tiles[x - 1][y])) {
			this.tiles[x][y] = "#";
		    }
		    // Eastbound
		    if ("R".equals(this.tiles[x - 1][y]) && "#".equals(this.tiles[x + 1][y])) {
		    ...
		    // Southbound
		    if ("R".equals(this.tiles[x][y - 1]) && "#".equals(this.tiles[x][y + 1])) {
			this.tiles[x][y] = "#";
		    }
		    // Northbound
		    if ("R".equals(this.tiles[x][y + 1]) && "#".equals(this.tiles[x][y - 1])) {
	...
	for (int x = 0; x < this.size.width; x++) {
		...
		if ("H".equals(tile)) {
		    // Westbound
		    if ("R".equals(this.tiles[x + 1][y]) && "R".equals(this.tiles[x - 1][y])) {
			this.tiles[x][y] = "D";
		    }
		    // Eastbound
		    if ("R".equals(this.tiles[x - 1][y]) && "R".equals(this.tiles[x + 1][y])) {
		    ...
		    // Southbound
		    if ("R".equals(this.tiles[x][y - 1]) && "R".equals(this.tiles[x][y + 1])) {
			this.tiles[x][y] = "D";
		    }
		    // Northbound
		    if ("R".equals(this.tiles[x][y + 1]) && "R".equals(this.tiles[x][y - 1])) {
	...
	for (int x = 0; x < this.size.width; x++) {
		...
		if ("H".equals(tile)) {
		    // Southbound
		    if ("R".equals(this.tiles[x][y - 1]) && "H".equals(this.tiles[x][y + 1])) {
			this.tiles[x][y] = "D";
		    }
		    // Northbound
		    if ("R".equals(this.tiles[x][y + 1]) && "H".equals(this.tiles[x][y - 1])) {
		    ...
		    // Westbound
		    if ("R".equals(this.tiles[x + 1][y]) && "H".equals(this.tiles[x - 1][y])) {
			this.tiles[x][y] = "D";
		    }
		    // Eastbound
		    if ("R".equals(this.tiles[x - 1][y]) && "H".equals(this.tiles[x + 1][y])) {
	...
	final OpenSpace open = new OpenSpace();
	final FantastleObjectModel[] roomReplacers = new FantastleObjectModel[] { new ArmorShop(), new BankShop(),
		new BonusShop(), new ElementalShop(), new HealShop(), new ItemShop(), new NecklaceShop(),
		new RegenerateShop(), new SpellShop(), new WeaponShop(), new Pit(), new SuperPit(), new Spring(),
		new SuperSpring(), open, open, open, open, open, open, open, open, open, open, open, open, open, open,
	...
	for (int x = 0; x < this.size.width; x++) {
		...
		if ("#".equals(tile)) {
		    ...
		    final int rIndex = RandomRange.generate(0, roomReplacers.length - 1);
	...
	int fF = floor;
	final int floors = this.getFloors();
	if (fF < 0) {
	    fF += floors;
	    while (fF < 0) {
		fF += floors;
	...
	for (x = 0; x < this.getColumns(); x++) {
	    for (y = 0; y < this.getRows(); y++) {
		final FantastleObjectModel obj = this.getCell(y, x, floor, Layers.OBJECT);
		obj.tickTimer();
	    }
	}
    }
    ...
    // File I/O
    public void writeLayeredTower(final DataIOWriter writer) throws IOException {
	int y, x, z, e;
    ...
    public static LayeredTower readLayeredTowerV1(final DataIOReader reader) throws IOException {
	...
	for (x = 0; x < lt.getColumns(); x++) {
		...
		for (z = 0; z < lt.getFloors(); z++) {
		    for (e = 0; e < Layers.COUNT; e++) {
			lt.setCell(GameObjects.readObject(reader, WorldVersions.LATEST), y, x, z, e);
    ...
    public void writeSavedTowerState(final DataIOWriter writer) throws IOException {
	int x, y, z, e;
	writer.writeInt(this.getColumns());
	writer.writeInt(this.getRows());
	writer.writeInt(this.getFloors());
    ...
    public void readSavedTowerState(final DataIOReader reader, final int formatVersion) throws IOException {
	int x, y, z, e, sizeX, sizeY, sizeZ;
	sizeX = reader.readInt();
	sizeY = reader.readInt();
	sizeZ = reader.readInt();
	this.savedTowerState = new LowLevelWorldDataStore(sizeY, sizeX, sizeZ, Layers.COUNT);
	for (x = 0; x < sizeY; x++) {
	    for (y = 0; y < sizeX; y++) {
		for (z = 0; z < sizeZ; z++) {
		    for (e = 0; e < Layers.COUNT; e++) {
			this.savedTowerState.setCell(GameObjects.readObject(reader, formatVersion), y, x, z, e);
		    }
		}
	    }
	}
