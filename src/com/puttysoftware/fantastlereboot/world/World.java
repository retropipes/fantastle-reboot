...
public class World {
	...
	this.basePath = System.getProperty("java.io.tmpdir") + File.separator + "FantastleReboot" + File.separator
		+ randomID + FileExtensions.getWorldTempExtensionWithPeriod();
	final File base = new File(this.basePath);
	final boolean success = base.mkdirs();
	if (!success) {
	    CommonDialogs.showErrorDialog("World temporary folder creation failed!", "FantastleReboot");
	}
    }
	...
	temp.addLevel(FantastleReboot.getBattleWorldSize(), FantastleReboot.getBattleWorldSize(), 1);
	return temp;
    }
	...
	if (this.activeLevel != level) {
	    ...
	    try (DataIOReader reader = this.getLevelReader()) {
		// Load new level
    ...
    public boolean cellRangeCheck(final int row, final int col, final int floor, final int level, final int extra) {
	return this.worldData.cellRangeCheck(row, col, floor, extra) && level >= World.MIN_LEVELS - 1
		&& level < World.MAX_LEVELS;
    }
	...
	if (Prefs.isWorldGeneratorConstrainedRandom()) {
	    ...
	    this.worldData.fillRandomlyTwister(this.activeLevel);
	} else {
	    this.worldData.fillRandomlyConstrained(this, this.activeLevel);
	}
    }
	...
	return (this.globalVisionMode | VisionModes.EXPLORE) == this.globalVisionMode;
    }

    public boolean isGlobalVisionModeFieldOfView() {
	return (this.globalVisionMode | VisionModes.FIELD_OF_VIEW) == this.globalVisionMode;
    }

    public boolean isGlobalVisionModeFixedRadius() {
	return (this.globalVisionMode | VisionModes.FIXED_RADIUS) == this.globalVisionMode;
    }
	...
	try (DataIOReader metaReader = DataIOFactory.createTagReader(m.basePath + File.separator + "metafile.xml",
		"world")) {
	    // Read metafile
	...
	// Create data reader
	try (DataIOReader dataReader = m.getLevelReader()) {
	    // Read data
    ...
    private DataIOReader getLevelReader() throws IOException {
	return DataIOFactory.createTagReader(this.basePath + File.separator + "level" + this.activeLevel + ".xml",
		"level");
    }

    private int readWorldMetafile(final DataIOReader reader) throws IOException {
    ...
    private void readWorldLevel(final DataIOReader reader) throws IOException {
	this.readWorldLevel(reader, WorldVersions.LATEST);
    }

    private void readWorldLevel(final DataIOReader reader, final int formatVersion) throws IOException {
	...
	try {
	    ...
	    try (DataIOWriter metaWriter = DataIOFactory
		    .createTagWriter(this.basePath + File.separator + "metafile.xml", "world")) {
		// Write metafile
    ...
    private void writeWorldMetafile(final DataIOWriter writer) throws IOException {
	if (this.prefixHandler != null) {
    ...
    private void writeWorldLevel(final DataIOWriter writer) throws IOException {
	// Write the level
