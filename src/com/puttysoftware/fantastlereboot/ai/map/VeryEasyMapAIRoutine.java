...
class VeryEasyMapAIRoutine extends AbstractMapAIRoutine {
	...
	if (this.spellCheck(ac)) {
		...
		} else if (CommonMapAIRoutines.check(ac, VeryEasyMapAIRoutine.DRAIN_CHANCE)) {
		    // Drain MP
		...
		if (CommonMapAIRoutines.check(ac, VeryEasyMapAIRoutine.FLEE_CHANCE)) {
		    ...
		    // Look further
		    for (int x = CommonMapAIRoutines.MIN_VISION + 1; x <= VeryEasyMapAIRoutine.MAX_VISION; x++) {
		...
		if (ac.getCharacter().canAct()) {
			...
			if (ac.getCharacter().getCreature()
				.getCurrentHP() > ac.getCharacter().getCreature().getMaximumHP()
					* VeryEasyMapAIRoutine.HEAL_THRESHOLD / 100) {
			    // Do not need healing
			    return false;
			}
		    }
