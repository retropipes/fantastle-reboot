...
class NormalMapAIRoutine extends AbstractMapAIRoutine {
	...
	if (this.spellCheck(ac)) {
	    ...
	    if (there != null) {
		if (CommonMapAIRoutines.check(ac, NormalMapAIRoutine.STEAL_CHANCE)) {
		    // Steal
		    return AIRoutine.ACTION_STEAL;
		} else if (CommonMapAIRoutines.check(ac, NormalMapAIRoutine.DRAIN_CHANCE)) {
		    // Drain MP
		    return AIRoutine.ACTION_DRAIN;
		} else {
		    // Something hostile is nearby, so attack it
		    if (ac.getCharacter().canAct()) {
			this.moveX = there.x;
			this.moveY = there.y;
		...
		if (CommonMapAIRoutines.check(ac, NormalMapAIRoutine.FLEE_CHANCE)) {
		    ...
		    // Look further
		    for (int x = CommonMapAIRoutines.MIN_VISION + 1; x <= NormalMapAIRoutine.MAX_VISION; x++) {
