...
class VeryHardMapAIRoutine extends AbstractMapAIRoutine {
	...
	if (this.spellCheck(ac)) {
		...
		} else if (CommonMapAIRoutines.check(ac, VeryHardMapAIRoutine.DRAIN_CHANCE)) {
		    // Drain MP
		...
		if (CommonMapAIRoutines.check(ac, VeryHardMapAIRoutine.FLEE_CHANCE)) {
		    ...
		    // Look further
		    for (int x = CommonMapAIRoutines.MIN_VISION + 1; x <= VeryHardMapAIRoutine.MAX_VISION; x++) {
		...
		if (ac.getCharacter().canAct()) {
			...
			if (ac.getCharacter().getCreature()
				.getCurrentHP() > ac.getCharacter().getCreature().getMaximumHP()
					* VeryHardMapAIRoutine.HEAL_THRESHOLD / 100) {
			    // Do not need healing
			    return false;
			}
		    }
