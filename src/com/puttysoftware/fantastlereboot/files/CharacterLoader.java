...
public class CharacterLoader {
	...
	try (DataIOReader loader = DataIOFactory.createTagReader(loadPath, "character")) {
    ...
    public static PartyMember readCharacter(final DataIOReader reader) throws IOException {
	...
	final ItemInventory ii = ItemInventory.readItemInventory(reader);
	...
	if (version >= CharacterVersions.V2) {
	    ...
	    avatarModel = new AvatarImageModel(avatarFamily, avatarHair, avatarSkin, avatarBody, avatarPants,
		    avatarShoes, avatarEyes);
	    pm = PartyManager.getNewPCInstance(ii, r, j, f, n, avatarFamily, avatarModel);
	} else {
	    // Version 1
	    avatarModel = new AvatarImageModel(avatarLegacyFamily, avatarHair, avatarSkin);
	    pm = PartyManager.getNewPCInstance(ii, r, j, f, n, avatarLegacyFamily, avatarModel);
	}
