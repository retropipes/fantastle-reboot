...
public final class WorldFileManager {
    ...
    public static void handleDeferredSuccess(final boolean value, final boolean versionError, final File triedToLoad) {
	if (value) {
	...
	if (saved) {
		...
		final String returnVal = CommonDialogs.showInputDialog("Select a World", "Load World", choices,
			choices[0]);
		if (returnVal != null) {
		    int index = -1;
		    for (int x = 0; x < choices.length; x++) {
		    ...
		    if (index != -1) {
			final File file = new File(gameDir + File.separator + rawChoices[index]);
	...
	if (!FilenameChecker
		.isFilenameOK(WorldFileManager.getNameWithoutExtension(WorldFileManager.getFileNameOnly(filename)))) {
	...
	while (!FilenameChecker.isFilenameOK(returnVal)) {
		...
		final File file = new File(WorldFileManager.getWorldDirectory() + returnVal + extension);
		filename = file.getAbsolutePath();
		if (!FilenameChecker.isFilenameOK(returnVal)) {
		    CommonDialogs.showErrorDialog("The file name you entered contains illegal characters.\n"
			    ...
			    + "Files named con, nul, or prn are illegal, as are files\n"
			    + "named com1 through com9 and lpt1 through lpt9.", "Save World");
		} else {
		    // Make sure folder exists
		    if (!file.getParentFile().exists()) {
			final boolean okay = file.getParentFile().mkdirs();
			if (!okay) {
			    FantastleReboot.exception(new IOException("Cannot create game folder!"));
			}
		    }
