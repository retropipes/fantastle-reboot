...
public final class GameFileManager {
	...
	if (!FilenameChecker
		.isFilenameOK(GameFileManager.getNameWithoutExtension(GameFileManager.getFileNameOnly(filename)))) {
	...
	while (!FilenameChecker.isFilenameOK(returnVal)) {
		...
		final File file = new File(GameFileManager.getGameDirectory() + returnVal + extension);
		filename = file.getAbsolutePath();
