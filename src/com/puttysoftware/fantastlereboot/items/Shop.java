...
public class Shop {
    ...
    public static int getHealingCost(final int x, final int y, final int z, final int k) {
	return (int) (Math.log10(x) * (z - y + Math.log10((double) k + 1)));
    }

    public static int getRegenerationCost(final int x, final int y, final int z, final int k) {
	final int diff = z - y;
	if (diff == 0) {
	    return 0;
	} else {
	    final int cost = (int) (Math.log(x) / Math.log(2) * (z - y + Math.log((double) k + 1) / Math.log(2)));
	    if (cost < 1) {
		return 1;
	    } else {
		return cost;
	...
	final int cost = 15 * i * i + 15 * i + (int) Math.sqrt(Shop.getEquipmentCost(x));
	if (cost < 0) {
	    return 0;
	} else {
	    return cost;
	...
	if (this.type == ShopTypes.WEAPONS) {
	    ...
	    this.typeIndex = PartyManager.getParty().getLeader().getFaith().getFaithID();
	}
	if (this.typeChoices != null) {
	    this.typeResult = CommonDialogs.showInputDialog(this.getGoldTotals() + "Select Type",
		    this.getShopNameFromType(), this.typeChoices, this.typeChoices[this.typeDefault]);
	...
	if (this.type == ShopTypes.WEAPONS) {
		...
		this.choices = EquipmentFactory.createOneHandedWeaponNames(playerCharacter.getJob().getJobID());
		// Choose Hand
		this.handChoices = WeaponConstants.getHandChoices();
		this.handDefault = 0;
		this.handResult = CommonDialogs.showInputDialog(this.getGoldTotals() + "Select Hand",
			this.getShopNameFromType(), this.handChoices, this.handChoices[this.handDefault]);
		...
		this.choices = EquipmentFactory.createTwoHandedWeaponNames(playerCharacter.getJob().getJobID());
	    }
	} else if (this.type == ShopTypes.ARMOR) {
	    this.choices = EquipmentFactory.createArmorNames(this.typeIndex);
	} else if (this.type == ShopTypes.HEALER || this.type == ShopTypes.REGENERATOR) {
	    ...
	    this.choices = PartyManager.getParty().getLeader().getItems().generateEquipmentEnhancementStringArray();
	    this.index = 0;
	} else if (this.type == ShopTypes.FAITH_POWERS) {
	    this.choices = PartyManager.getParty().getLeader().getItems().generateEquipmentEnhancementStringArray();
	    this.index = 0;
	...
	final PartyMember playerCharacter = PartyManager.getParty().getLeader();
	// Check
	if (this.type == ShopTypes.HEALER && playerCharacter.getCurrentHP() == playerCharacter.getMaximumHP()) {
		...
		.getSpellsKnownCount() == playerCharacter.getSpellBook().getSpellCount()) {
	    CommonDialogs.showDialog("There are no more spells to learn.");
	    return false;
	}
	this.result = CommonDialogs.showInputDialog(this.getGoldTotals() + "Select", this.getShopNameFromType(),
	...
	if (this.type == ShopTypes.WEAPONS || this.type == ShopTypes.ARMOR) {
	    ...
	    this.cost = Shop.getHealingCost(playerCharacter.getLevel(), playerCharacter.getCurrentHP(),
		    playerCharacter.getMaximumHP(), playerCharacter.getKills());
	} else if (this.type == ShopTypes.REGENERATOR) {
	    this.cost = Shop.getRegenerationCost(playerCharacter.getLevel(), playerCharacter.getCurrentMP(),
		    playerCharacter.getMaximumMP(), playerCharacter.getKills());
	} else if (this.type == ShopTypes.BANK) {
	    String stage4Result = "";
	    while (stage4Result.equals("")) {
		stage4Result = CommonDialogs.showTextInputDialog(this.getGoldTotals() + this.result + " How Much?",
		...
		try {
		    ...
		    CommonDialogs.showErrorDialog("Amount must be greater than zero.", this.getShopNameFromType());
		    stage4Result = "";
		}
	    }
	} else if (this.type == ShopTypes.SPELLS) {
	    ...
	    final Equipment old = playerCharacter.getItems().getEquipmentInSlot(this.index);
	    if (old != null) {
		if (old.isTwoHanded()) {
		    this.twoHanded = true;
		}
		...
		if (this.cost == 0) {
		    ...
		    CommonDialogs.showErrorDialog("That equipment cannot be enhanced any further, sorry!",
			    this.getShopNameFromType());
		    return false;
		}
	    } else {
		CommonDialogs.showErrorDialog("Nothing is equipped there!", this.getShopNameFromType());
		return false;
	    }
	} else if (this.type == ShopTypes.FAITH_POWERS) {
	    final Equipment old = playerCharacter.getItems().getEquipmentInSlot(this.index);
		...
		this.item = EquipmentFactory.createFaithPoweredEquipment(old, this.typeIndex);
		this.cost = Shop.getEnhancementCost(bonus, power);
		if (this.cost == 0) {
		    // Equipment is maxed out on Faith Powers
		    CommonDialogs.showErrorDialog("That equipment cannot be Faith Powered any further, sorry!",
			    this.getShopNameFromType());
		    return false;
		}
	    } else {
	...
	if (this.type != ShopTypes.BANK) {
	    ...
	    final int stage4Confirm = CommonDialogs.showConfirmDialog(
		    "This will cost " + this.cost + " Gold. Are you sure?", this.getShopNameFromType());
	...
	if (this.type == ShopTypes.BANK) {
		...
		if (playerCharacter.getGold() < this.cost) {
		    CommonDialogs.showErrorDialog("Not Enough Gold!", this.getShopNameFromType());
		    return false;
		}
	    } else {
	...
	if (this.type == ShopTypes.WEAPONS) {
	    ...
	    if (this.typeResult.equals(this.typeChoices[0])) {
		final Equipment bought = EquipmentFactory.createOneHandedWeapon(this.index,
			playerCharacter.getJob().getJobID(), 0);
		playerCharacter.getItems().equipOneHandedWeapon(playerCharacter, bought, this.handIndex, true);
	    } else {
		final Equipment bought = EquipmentFactory.createTwoHandedWeapon(this.index,
			playerCharacter.getJob().getJobID(), 0);
		playerCharacter.getItems().equipTwoHandedWeapon(playerCharacter, bought, true);
	    }
	} else if (this.type == ShopTypes.ARMOR) {
	    playerCharacter.offsetGold(-this.cost);
	    final Equipment bought = EquipmentFactory.createArmor(this.index, this.typeIndex, 0);
	    ...
	    playerCharacter.getItems().equipArmor(playerCharacter, (Socks) Shop.this.item, true);
	} else if (Shop.this.type == ShopTypes.ENHANCEMENTS) {
	    playerCharacter.offsetGold(-Shop.this.cost);
	    playerCharacter.getItems().setEquipmentInSlot(Shop.this.index, (Equipment) Shop.this.item);
	    if (Shop.this.twoHanded) {
		if (Shop.this.index == EquipmentSlotConstants.SLOT_MAINHAND) {
		    playerCharacter.getItems().setEquipmentInSlot(EquipmentSlotConstants.SLOT_OFFHAND,
			    (Equipment) Shop.this.item);
		} else if (Shop.this.index == EquipmentSlotConstants.SLOT_OFFHAND) {
		    playerCharacter.getItems().setEquipmentInSlot(EquipmentSlotConstants.SLOT_MAINHAND,
			    (Equipment) Shop.this.item);
		}
	    }
