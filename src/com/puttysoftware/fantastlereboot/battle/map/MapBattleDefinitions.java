...
public class MapBattleDefinitions {
	...
	private final boolean partyDead;
	private boolean enemiesAlive;
	private boolean enemiesGone;
	private final boolean enemiesDead;
	...
	public BattleState() {
	    ...
	    final MapBattleDefinitions mbd = MapBattleDefinitions.this;
	    for (final BattleCharacter battler : mbd.battlers) {
		if (battler != null) {
		    final int teamID = battler.getTeamID();
		    final boolean alive = battler.getCreature().isAlive();
		    final boolean isActive = battler.isActive();
	...
	final RandomRange randY = new RandomRange(0, this.battleWorld.getColumns() - 1);
	int rx, ry;
	for (final BattleCharacter battler : this.battlers) {
		...
		if (battler.getCreature().isAlive() && !battler.isLocationSet()) {
		    rx = randX.generate();
		    ry = randY.generate();
		    FantastleObjectModel obj = this.battleWorld.getCell(rx, ry, 0, Layers.OBJECT);
		    while (obj.isSolid()) {
			rx = randX.generate();
			ry = randY.generate();
	...
	int nnc = 0;
	for (final BattleCharacter battler : this.battlers) {
	    if (battler != null && battler.isActive() && battler.getCreature().isAlive()) {
	...
	for (final BattleCharacter battler : this.battlers) {
		...
		final String effectMessages = battler.getCreature().getAllCurrentEffectMessages();
	...
	for (final BattleCharacter battler : this.battlers) {
	    if (battler != null && battler.isActive() && battler.getCreature().isAlive() && !battler.isMarked()) {
		final Creature creature = battler.getCreature();
		final int creatureSpeed = (int) creature.getEffectedStat(StatConstants.STAT_AGILITY);
