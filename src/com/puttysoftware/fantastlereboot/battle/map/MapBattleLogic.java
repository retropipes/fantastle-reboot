...
public class MapBattleLogic extends Battle {
	...
	// Set First Active
	this.newRound = this.setNextActive();
	// Clear status message
	this.clearStatusMessage();
	// Start Battle
	this.battleGUI.getViewManager().setViewingWindowCenterX(this.mbd.getActiveCharacter().getY());
	this.battleGUI.getViewManager().setViewingWindowCenterY(this.mbd.getActiveCharacter().getX());
	...
	final MapBattleDefinitions.BattleState state = this.mbd.getBattleState();
	...
	if (this.mbd != null && this.mbd.getActiveCharacter() != null
		...
		final int action = active.getCreature().getMapAI()
			.getNextAction(this.mbd.getActiveCharacter().getCreature(), this.mbd.getActiveAIContext());
		switch (action) {
		case AIRoutine.ACTION_MOVE:
		    ...
		    final AbstractDamageEngine activeDE = activeTID == Creature.TEAM_PARTY ? this.ede : this.pde;
		    this.lastAIActionResult = this.updatePositionInternal(x, y, true, this.mbd.getActiveCharacter(),
			    activeDE);
		    active.getCreature().getMapAI().setLastResult(this.lastAIActionResult);
		    break;
	...
	final int action = this.auto.getNextAction(acting.getCreature(), acting.getAIContext());
	switch (action) {
	case AIRoutine.ACTION_MOVE:
    ...
    private void displayRoundResults(final Creature theEnemy, final Creature active,
	...
	if (this.damage == 0) {
		...
		displayDamageString = displayDamagePrefix + activeName + " hits " + enemyName + " for " + damageString
			+ " damage!" + activeDE.getDamageString();
		if (isParty) {
    ...
    private void computeDamage(final Creature theEnemy, final Creature acting, final AbstractDamageEngine activeDE) {
	// Compute Damage
	this.damage = 0;
	...
	final boolean res = this.mbd.setNextActive();
	if (res) {
	    // AI Check
	...
	final int activeTID = this.mbd.getActiveCharacter().getTeamID();
	final AbstractDamageEngine activeDE = activeTID == Creature.TEAM_PARTY ? this.ede : this.pde;
	this.updatePositionInternal(x, y, true, this.mbd.getActiveCharacter(), activeDE);
    }
	...
	if (this.mbd.getActiveCharacter().getCurrentActions() > 0) {
	    ...
	    final MapBattleArrowTask at = new MapBattleArrowTask(x, y, this.mbd.getBattleWorld(),
		    this.mbd.getActiveCharacter());
	    at.start();
	} else {
	...
	if (hit != null && !hit.getCreature().isAlive()) {
	    ...
	    this.mbd.getBattleWorld().setCell(new OpenSpace(), hit.getX(), hit.getY(), 0, Layers.OBJECT);
	}
	// Check result
	final BattleResults currResult = this.getResult();
	if (currResult != BattleResults.IN_PROGRESS) {
    ...
    @Override
    public boolean handleDeath(final BattleCharacter active, final BattleCharacter enemy) {
	...
	if (enemy != null && !enemy.getCreature().isAlive()) {
	    ...
	    this.mbd.getBattleWorld().setCell(new OpenSpace(), enemy.getX(), enemy.getY(), 0, Layers.OBJECT);
	}
	// Handle self death
	if (!active.getCreature().isAlive()) {
	    // Run death hook
	    ...
	    active.deactivate();
	    // Remove dead character from battle
	    this.mbd.getBattleWorld().setCell(new OpenSpace(), active.getX(), active.getY(), 0, Layers.OBJECT);
	...
	if (next != null && nextGround != null && currGround != null) {
	    if (!next.isSolid()) {
		if (useAP && this.getActiveActionCounter() >= AIContext.getDefaultAPCost() || !useAP) {
		    // Move
		...
		if (next instanceof BattleCharacter) {
			...
			// Do damage
			this.computeDamage(bc.getCreature(), active.getCreature(), activeDE);
			// Handle low health for party members
			if (bc.getCreature().isAlive() && bc.getTeamID() == Creature.TEAM_PARTY
				&& bc.getCreature().getCurrentHP() <= bc.getCreature().getMaximumHP() * 3 / 10) {
	    ...
	    m.setCell(new OpenSpace(), active.getX(), active.getY(), 0, Layers.OBJECT);
	...
	final String response = CommonDialogs.showInputDialog("Pick A Target", "Target", pickNames, pickNames[0]);
    ...
    @Override
    public Creature getEnemy(final int teamID) {
	...
	if (activeBC.canAct()) {
	    final BattleCharacter anyEnemy = this.mbd.getFirstBattlerNotOnTeam(activeBC.getTeamID());
	    if (anyEnemy == null) {
		// Failed - nobody to use on
		this.setStatusMessage(
			activeBC.getName() + " tries to cast a spell, but nobody is there to cast it on!");
		return false;
	    }
	    if (!active.hasMapAI()) {
	...
	if (activeBC.canAct()) {
	    ...
	    if (anyEnemy == null) {
		// Failed - nobody to use on
		this.setStatusMessage(activeBC.getName() + " tries to use an item, but nobody is there to use it on!");
		return false;
	    }
	    if (!active.hasMapAI()) {
		// Active character has no AI, or AI is turned off
	...
	if (activeBC.canAct()) {
	    ...
	    final int stealChance = StatConstants.CHANCE_STEAL
		    + active.getJob().getAttribute(JobConstants.ATTRIBUTE_STEAL_SUCCESS_MODIFIER);
	    activeBC.act(Battle.AP_STEAL);
	    final BattleCharacter anyEnemy = this.mbd.getFirstBattlerNotOnTeam(activeBC.getTeamID());
	    if (anyEnemy == null) {
		// Failed - nobody to steal from
		this.setStatusMessage(activeBC.getName() + " tries to steal, but nobody is there to steal from!");
		return false;
	    }
	    final Creature activeEnemy = anyEnemy.getCreature();
		...
		if (stealAmount == 0) {
		    this.setStatusMessage(activeBC.getName() + " tries to steal, but no Gold is left to steal!");
		    return false;
		} else {
		    activeEnemy.offsetGold(-stealAmount);
		    active.offsetGold(stealAmount);
		    this.setStatusMessage(
			    activeBC.getName() + " tries to steal, and successfully stole " + stealAmount + " Gold!");
		    return true;
		}
	    } else {
		// Failed
		this.setStatusMessage(activeBC.getName() + " tries to steal, but fails!");
		return false;
	    }
	} else {
	...
	if (activeBC.canAct()) {
	    ...
	    drainChance = StatConstants.CHANCE_DRAIN
		    + active.getJob().getAttribute(JobConstants.ATTRIBUTE_DRAIN_SUCCESS_MODIFIER);
	    activeBC.act(Battle.AP_DRAIN);
	    final BattleCharacter anyEnemy = this.mbd.getFirstBattlerNotOnTeam(activeBC.getTeamID());
	    if (anyEnemy == null) {
		// Failed - nobody to drain from
		this.setStatusMessage(activeBC.getName() + " tries to drain, but nobody is there to drain from!");
		return false;
	    }
	    final Creature activeEnemy = anyEnemy.getCreature();
	    if (drainChance <= 0) {
		// Failed
		this.setStatusMessage(activeBC.getName() + " tries to drain, but fails!");
		return false;
		...
		if (drainAmount == 0) {
		    this.setStatusMessage(activeBC.getName() + " tries to drain, but no MP is left to drain!");
		    return false;
		} else {
		    activeEnemy.offsetCurrentMP(-drainAmount);
		    active.offsetCurrentMP(drainAmount);
		    this.setStatusMessage(
			    activeBC.getName() + " tries to drain, and successfully drains " + drainAmount + " MP!");
		    return true;
		}
	    } else {
		final RandomRange chance = new RandomRange(0, 100);
		...
		if (randomChance <= drainChance) {
			...
			activeEnemy.offsetCurrentMP(-drainAmount);
			active.offsetCurrentMP(drainAmount);
			this.setStatusMessage(activeBC.getName() + " tries to drain, and successfully drains "
				+ drainAmount + " MP!");
			return true;
		    }
		} else {
    ...
    @Override
    public void redrawOneBattleSquare(final int x, final int y, final FantastleObjectModel obj3) {
	if (!Modes.inBattle()) {
	    // Abort
	    return;
	}
    ...
    @Override
    public void doResult(final BattleResults result) {
	if (!Modes.inBattle()) {
	    // Abort
	    return;
	}
	...
	if (!this.resultDoneAlready) {
	    ...
	    final boolean bossFlag = this.mbd.getFirstBattlerOnTeam(Creature.TEAM_BOSS) != null;
		...
		PartyManager.getParty().distributeVictorySpoils(this.battleExp, this.battleGold);
		if (bossFlag) {
		    rewardsFlag = true;
		}
	    } else if (result == BattleResults.PERFECT) {
		SoundPlayer.playSound(SoundIndex.VICTORY, SoundGroup.BATTLE);
		CommonDialogs.showTitledDialog("The party is victorious, and escaped unharmed!", "Perfect Victory!");
		final long pBattleExp = Prefs.getPerfectBattleExp(this.battleExp);
		final int pBattleGold = Prefs.getPerfectBattleGold(this.battleGold);
		PartyManager.getParty().distributeVictorySpoils(pBattleExp, pBattleGold);
		if (bossFlag) {
		    rewardsFlag = true;
		}
		...
		CommonDialogs.showTitledDialog("The party has been annihilated!", "Annihilation!");
	...
	return !this.mbd.getBattleWorld().getCell(inX, inY, 0, Layers.OBJECT).isSolid();
    }
}
