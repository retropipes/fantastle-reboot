...
class MapBattleGUI {
    ...
    // Constructors
    MapBattleGUI() {
	this.battleFrame = MainWindow.mainWindow();
	...
	for (x = xView; x <= xlView; x++) {
		...
		if (mbd.getBattleWorld().cellRangeCheck(y, x, 0)) {
		    final FantastleObjectModel obj1 = mbd.getBattleWorld().getCell(y, x, 0, Layers.GROUND);
		    final FantastleObjectModel obj2 = mbd.getBattleWorld().getCell(y, x, 0, Layers.OBJECT);
		    ...
		    this.drawGrid.setImageCell(ImageCompositor.composite(cacheName, icon1, icon2), xFix, yFix);
		} else {
    ...
    void redrawOneBattleSquare(final MapBattleDefinitions mbd, final int x, final int y,
	...
	if (mbd.getBattleWorld().cellRangeCheck(y, x, 0)) {
	    ...
	    this.drawGrid.setImageCell(ImageCompositor.composite(cacheName, icon1, icon2, icon3), xFix, yFix);
	    this.battlePane.repaint();
	}
	this.battleFrame.pack();
    }

    private static String generateCacheName(final FantastleObjectModel... objects) {
	...
	final BattleCharacter activeCharacter = mbd.getActiveCharacter();
	...
	final BufferedImageIcon darknessImage = ObjectImageLoader.load(ObjectImageIndex.DARKNESS);
