...
public class MapBattleArrowTask extends Thread {
    ...
    // Constructors
    public MapBattleArrowTask(final int newX, final int newY, final World world, final BattleCharacter ac) {
	this.x = newX;
	this.y = newY;
	...
	try {
	    ...
	    final int newDir = DirectionsResolver.resolve(incX, incY);
	    final FantastleObjectModel a = ArrowFactory.createArrow(ArrowType.WOODEN, newDir);
		...
		if (res) {
			...
			final int damage = FaithManager.getFaithAdjustedDamage(shooter.getFaithID(),
				target.getFaithID(), 5);
		...
		// Clear arrow
		app.getBattle().redrawOneBattleSquare(px + cumX, py + cumY, new OpenSpace());
		cumX += incX;
		cumY += incY;
		if (m.cellRangeCheck(px + cumX, py + cumY, 0)) {
		    o = m.getCell(px + cumX, py + cumY, 0, Layers.OBJECT);
