...
class VeryEasyDamageEngine extends AbstractDamageEngine {
	...
	if (this.fumble) {
		...
		if (rEvade < aEvade) {
			...
			rDamage = new RandomRange(VeryEasyDamageEngine.MULTIPLIER_MIN,
			...
			for (int z = 0; z < fc; z++) {
			    ...
			    faithMultiplier += VeryEasyDamageEngine.FAITH_INCREMENT_2H * fpl;
			}
		    } else {
			if (mainHand != null) {
			    for (int z = 0; z < fc; z++) {
				final int fpl = mainHand.getFaithPowerLevel(z);
				faithMultiplier += VeryEasyDamageEngine.FAITH_INCREMENT * fpl;
			    }
			}
			...
			for (int z = 0; z < fc; z++) {
			    ...
			    faithDR -= fpl * VeryEasyDamageEngine.FAITH_DR_INCREMENT;
			}
		    }
		    if (offHand != null
		    ...
		    final double unadjustedDamage = rawDamage * multiplier * faithMultiplier
