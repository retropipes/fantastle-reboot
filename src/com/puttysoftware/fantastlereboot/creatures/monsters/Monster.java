...
public final class Monster extends Creature {
	...
	super(new ItemInventory(), teamID, FaithManager.getRandomFaith(), JobManager.getRandomJob(),
		RaceManager.getRandomRace());
	this.setMapAI(Monster.getInitialMapAI());
	final SpellBook spells = SpellBookManager.getEnemySpellBookByID(Prefs.getGameDifficulty());
	...
	final RandomRange r = new RandomRange(1,
		Math.max(this.getLevel() * Monster.getStatMultiplierForDifficulty(), 1));
	return r.generate() + super.getInitialStrength();
    }

    @Override
    protected int getInitialBlock() {
	final RandomRange r = new RandomRange(0, this.getLevel() * Monster.getStatMultiplierForDifficulty());
