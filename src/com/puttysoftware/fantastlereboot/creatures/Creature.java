...
public abstract class Creature {
    ...
    // Constructor
    protected Creature(final ItemInventory ii, final int tid, final Faith f, final Job j, final Race r) {
	this.faith = f;
	this.job = j;
	this.race = r;
	this.teamID = tid;
	...
	final double agilityContrib = Math.min(this.getEffectedStat(StatConstants.STAT_AGILITY),
		Creature.MAX_AGILITY_CONTRIB) / (Creature.MAX_AGILITY_CONTRIB / (double) Creature.FUMBLE_BASE)
		* StatConstants.FACTOR_AGILITY_FUMBLE;
	final double luckContrib = Math.min(this.getEffectedStat(StatConstants.STAT_LUCK), Creature.MAX_LUCK_CONTRIB)
		/ (Creature.MAX_LUCK_CONTRIB / (double) Creature.FUMBLE_BASE) * StatConstants.FACTOR_LUCK_FUMBLE;
	...
	final int value = (int) Math.sqrt(Math.ceil(
	...
	final double agilityContrib = Math.max(0, this.getEffectedStat(StatConstants.STAT_AGILITY))
		* StatConstants.FACTOR_AGILITY_EVADE;
	final double luckContrib = Math.max(0, this.getEffectedStat(StatConstants.STAT_LUCK))
	...
	final double strengthContrib = Math.max(0, this.getEffectedStat(StatConstants.STAT_STRENGTH))
	...
	return (int) (this.getEffectedStat(StatConstants.STAT_AGILITY) * StatConstants.FACTOR_AGILITY_SPEED
		- (this.items.getTotalEquipmentWeight() + this.items.getTotalInventoryWeight())
	...
	final Job j = this.getJob();
	final int base = StatConstants.GAIN_STRENGTH * level;
	final int raceFixed = r.getAttribute(RaceConstants.ATTRIBUTE_STRENGTH_PER_LEVEL) * level;
	final int jobFixed = j.getAttribute(JobConstants.ATTRIBUTE_STRENGTH_MODIFIER);
	final double raceMult = RaceConstants
		.getLookupTableEntry(r.getAttribute(RaceConstants.ATTRIBUTE_RANDOM_STRENGTH));
	...
	final Race r = this.getRace();
	final Job j = this.getJob();
	final int base = StatConstants.GAIN_BLOCK * level;
	final int raceFixed = r.getAttribute(RaceConstants.ATTRIBUTE_BLOCK_PER_LEVEL) * level;
	final int jobFixed = j.getAttribute(JobConstants.ATTRIBUTE_BLOCK_MODIFIER);
	final double raceMult = RaceConstants.getLookupTableEntry(r.getAttribute(RaceConstants.ATTRIBUTE_RANDOM_BLOCK));
	return (int) Math.round((base + raceFixed + jobFixed) * raceMult);
    }
	...
	final Race r = this.getRace();
	final Job j = this.getJob();
	final int base = StatConstants.GAIN_VITALITY * level;
	final int raceFixed = r.getAttribute(RaceConstants.ATTRIBUTE_VITALITY_PER_LEVEL) * level;
	final int jobFixed = j.getAttribute(JobConstants.ATTRIBUTE_VITALITY_MODIFIER);
	final double raceMult = RaceConstants
		.getLookupTableEntry(r.getAttribute(RaceConstants.ATTRIBUTE_RANDOM_VITALITY));
	return (int) Math.round((base + raceFixed + jobFixed) * raceMult);
    }
	...
	final Race r = this.getRace();
	final Job j = this.getJob();
	final int base = StatConstants.GAIN_INTELLIGENCE * level;
	final int raceFixed = r.getAttribute(RaceConstants.ATTRIBUTE_INTELLIGENCE_PER_LEVEL) * level;
	final int jobFixed = j.getAttribute(JobConstants.ATTRIBUTE_INTELLIGENCE_MODIFIER);
	final double raceMult = RaceConstants
		.getLookupTableEntry(r.getAttribute(RaceConstants.ATTRIBUTE_RANDOM_INTELLIGENCE));
	return (int) Math.round((base + raceFixed + jobFixed) * raceMult);
    }
	...
	final Race r = this.getRace();
	final Job j = this.getJob();
	final int base = StatConstants.GAIN_AGILITY * level;
	final int raceFixed = r.getAttribute(RaceConstants.ATTRIBUTE_AGILITY_PER_LEVEL) * level;
	final int jobFixed = j.getAttribute(JobConstants.ATTRIBUTE_AGILITY_MODIFIER);
	final double raceMult = RaceConstants
		.getLookupTableEntry(r.getAttribute(RaceConstants.ATTRIBUTE_RANDOM_AGILITY));
	return (int) Math.round((base + raceFixed + jobFixed) * raceMult);
    }
	...
	final Race r = this.getRace();
	final Job j = this.getJob();
	final int base = StatConstants.GAIN_LUCK * level;
	final int raceFixed = r.getAttribute(RaceConstants.ATTRIBUTE_LUCK_PER_LEVEL) * level;
	final int jobFixed = j.getAttribute(JobConstants.ATTRIBUTE_LUCK_MODIFIER);
	final double raceMult = RaceConstants.getLookupTableEntry(r.getAttribute(RaceConstants.ATTRIBUTE_RANDOM_LUCK));
	return (int) Math.round((base + raceFixed + jobFixed) * raceMult);
    }
	...
	if (this.stats[stat].getDynamism() != 0) {
	    final RandomRange r = new RandomRange(-this.stats[stat].getDynamism(), this.stats[stat].getDynamism());
