...
public class FaithManager {
	...
	dialogResult = CommonDialogs.showInputDialog("Select a Faith", "Select Faith", names, names[0]);
    ...
    public static int getFaithAdjustedDamage(final int attackerFaithID, final int defenderFaithID,
	    final int rawDamage) {
	FaithManager.initCachesIfNeeded();
	return rawDamage * FaithConstants.FAITH_NUMERATOR_CACHE[attackerFaithID][defenderFaithID]
	...
	if (!FaithManager.CACHES_INITED) {
		...
		final Color faithColor = new Color(colorData[0], colorData[1], colorData[2], colorData[3]);
		FaithConstants.FAITH_COLOR_CACHE.put(faithID, faithColor);
		FaithConstants.FAITH_NUMERATOR_CACHE[faithID] = DataLoader.loadFaithNumeratorData(faithID);
		FaithConstants.FAITH_DENOMINATOR_CACHE[faithID] = DataLoader.loadFaithDenominatorData(faithID);
