...
public class Party {
	...
	if (this.monsterLevel + offset > World.getMaxLevels() || this.monsterLevel + offset < 0) {
	    return;
	}
	...
	for (final PartyMember member : this.members) {
		...
		CommonDialogs.showTitledDialog(member.getName() + " reached level " + member.getLevel() + "!",
			"Level Up");
	    }
	}
    }
	...
	final long expPerMember = exp / numAlive;
	final long expLeftover = exp % numAlive;
	final int goldPerMember = gold / numAlive;
	final int goldLeftover = gold % numAlive;
	for (final PartyMember member : this.members) {
	    if (member.isAlive()) {
	...
	final String response = CommonDialogs.showInputDialog("Pick Leader", "Arrange Party", pickNames,
		pickNames[this.leaderID]);
	if (response != null) {
	    int x = 0;
	    for (final PartyMember member : this.members) {
