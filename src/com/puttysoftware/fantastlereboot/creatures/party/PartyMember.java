...
public class PartyMember extends Creature {
    ...
    // Constructors
    PartyMember(final ItemInventory ii, final Race r, final Job j, final Faith f, final String n, final int af,
	    final AvatarImageModel model) {
	...
	this.offsetStrength(StatConstants.GAIN_STRENGTH + r.getAttribute(RaceConstants.ATTRIBUTE_STRENGTH_PER_LEVEL));
	this.offsetBlock(StatConstants.GAIN_BLOCK + r.getAttribute(RaceConstants.ATTRIBUTE_BLOCK_PER_LEVEL));
	this.offsetVitality(StatConstants.GAIN_VITALITY + r.getAttribute(RaceConstants.ATTRIBUTE_VITALITY_PER_LEVEL));
	this.offsetIntelligence(
		StatConstants.GAIN_INTELLIGENCE + r.getAttribute(RaceConstants.ATTRIBUTE_INTELLIGENCE_PER_LEVEL));
	this.offsetAgility(StatConstants.GAIN_AGILITY + r.getAttribute(RaceConstants.ATTRIBUTE_AGILITY_PER_LEVEL));
	this.offsetLuck(StatConstants.GAIN_LUCK + r.getAttribute(RaceConstants.ATTRIBUTE_LUCK_PER_LEVEL));
	this.healAndRegenerateFully();
    }

    public void loadPartyMember(final int newLevel, final int chp, final int cmp, final int newGold, final int newLoad,
	    final long newExperience, final int bookID, final boolean[] known, final int k, final int pAtk,
	...
	return this.permanentHP * this.getJob().getAttribute(JobConstants.ATTRIBUTE_PERMANENT_HP_PER_POINT);
    }

    public int getPermanentMP() {
	return this.permanentMP * this.getJob().getAttribute(JobConstants.ATTRIBUTE_PERMANENT_MP_PER_POINT);
    }
	...
	return AvatarImageLoader.load(this.avatarFamilyID, this.imageModel.getRules());
    }
